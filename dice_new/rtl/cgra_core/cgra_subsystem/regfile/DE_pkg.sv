...
typedef struct packed {
    // all banks
    logic [(DICE_DATA_WIDTH*DICE_NUM_BANKS)-1:0] data;
    logic [DICE_NUM_BANKS-1:0] mask;
    logic [(DICE_TID_WIDTH*DICE_NUM_BANKS)-1:0] tid;

} ldst_wr_cmd;

typedef struct packed {
    // single bank
    logic [DICE_DATA_WIDTH-1:0] data;
    logic mask;
...
(
    ...
    for (int i = 0; i < DICE_NUM_BANKS; i++) begin
        wr_cmd[i].data = cmd.data[i*DICE_DATA_WIDTH +: DICE_DATA_WIDTH];
...
function automatic ldst_reg_wr_cmd assemble_ldst_wr
(
    input ldst_wr_cmd cmd
);
    ldst_reg_wr_cmd wr_data;
