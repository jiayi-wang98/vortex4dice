...
(
    ...
    // LOAD-RETIRE: asserted for one cycle when this bank's LDST writeback FIFO
    // pops a valid entry; carries the e-block id of the popped writeback so the
    // commit/retire path can decrement that block's pending-mem count.
    , output logic                       ldst_pop_o
    , output logic[EBLOCK_ID_W-1:0]      ldst_pop_e_block_id_o
...
// ---------------- CGRA buffer ----------------
  ...
  // LOAD-RETIRE pop: only assert when the LDST FIFO actually dequeues a valid,
  // masked writeback (CGRA has priority and does not consume FIFO entries).
  // Use ~ldst_empty (driven by the FIFO) rather than wb_valid_o, which the
  // dice_new reg_wr_buffer leaves undriven.
  assign ldst_pop_o            = pop_ldst & ~ldst_empty & ldst_wb.mask;
