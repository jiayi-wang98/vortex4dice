...
(
    ...
    input logic [REG_NUM-1:0] wb_regs_bitmap,   // One-hot register index of the returning load (per-register release)
    input logic clear_scoreboard,               // External CTA-clear: wipe all scoreboards on a true CTA-id change
    ...
    // Constant-register release slice for the shared constant scoreboard:
    // legacy per-eblock ld_dest CR bits OR per-register wb_regs_bitmap CR bits.
    logic [`DICE_CR_NUM-1:0] wb_const_release;
    assign wb_const_release =
        ld_dest_regs_bitmap[(`DICE_GPR_NUM + `DICE_CR_NUM - 1):`DICE_GPR_NUM]
      | wb_regs_bitmap     [(`DICE_GPR_NUM + `DICE_CR_NUM - 1):`DICE_GPR_NUM];
    ...
    // Combined scoreboard clear: internal per-eblock new-CTA dispatch OR the
    // external CTA-boundary clear. ORing keeps today's behavior unchanged when
    // clear_scoreboard is tied low.
    logic clear_scoreboard_all;
    assign clear_scoreboard_all = start_new_cta || clear_scoreboard;
    // syn_keep
    ...
    generate
        for (i = 0; i < NUM_SCOREBOARDS; i++) begin : gen_scoreboards
            scoreboard #(
                ...
                .wb_tid_bitmap(wb_tid_sb[i]),           // Each scoreboard gets its slice
                .ld_dest_regs_bitmap(ld_dest_regs_bitmap),         // Legacy per-eblock ld_dest release set
                .wb_regs_bitmap(wb_regs_bitmap),        // Per-register one-hot release of the returning load
                .wb_valid(wb_valid),
                .clear_scoreboard(clear_scoreboard_all),    // New-CTA dispatch OR external CTA-boundary clear
    ...
    constant_scoreboard #(.NUM_CONSTANT_REGS(`DICE_CR_NUM)) const_sb (
        ...
        // Constant release slice: OR the legacy per-eblock ld_dest CR bits with
        // the per-register one-hot wb_regs_bitmap CR bits. When the integrator
        // drives only one, the other is '0 and today's behavior is preserved.
        .wb_const_bitmap(wb_const_release),  // Constant register(s) to release
        .wb_valid(wb_valid && |wb_const_release),  // Valid only for constant regs
