...
#(
    ...
    input logic [REG_NUM-1:0]  ld_dest_regs_bitmap,       // Per-eblock ld_dest register set (legacy path, all loads of the eblock)
    input logic [REG_NUM-1:0]  wb_regs_bitmap,            // Per-register one-hot register of the returning load
    ...
    // Effective release register set: OR of the legacy per-eblock ld_dest set and
    // the new per-register one-hot wb_regs_bitmap. When the integrator drives only
    // one of the two, the other is '0 and today's behavior is preserved.
    logic [REG_NUM-1:0] wb_release_regs;
    assign wb_release_regs = ld_dest_regs_bitmap | wb_regs_bitmap;
    ...
    always_comb begin
        ...
        // Release ONLY {wb_tid_bitmap, wb_release_regs}; with per-register
        // release wb_release_regs is the single returning load's register, with
        // the legacy per-eblock path it is the full ld_dest set (OR of both).
        if (wb_valid) begin
            for (int j = 0; j < THREADS_PER_SCOREBOARD; j++) begin
                if (wb_tid_bitmap[j]) begin
                    // Release the specific register (set to 0)
                    update_data[j] = update_data[j] & ~wb_release_regs;
                    update_mask[j] = update_mask[j] |  wb_release_regs;
                end
            end
        end
        ...
        if (rsv_valid && wb_valid && wb_tid_bitmap[rsv_tid]) begin
            // Fuse the operations
            update_data[rsv_tid] = input_regs_map;
            update_mask[rsv_tid] = input_regs_map | wb_release_regs;
