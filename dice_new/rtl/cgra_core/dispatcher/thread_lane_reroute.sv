...
(
    ...
    input logic [CHUNK_ADDR_WIDTH-1:0] chunk_base_addr, // Base address for the chunk (10 bits)
    ...
    output logic [2*DICE_TID_WIDTH-1:0] fifo_data_0, // FIFO data outputs
    ...
    output logic [NUM_LANES-1:0] fifo_data_valid          // FIFO empty signal
);

    logic [NUM_LANES-1:0] valid;
    assign valid = {valid_3, valid_2, valid_1, valid_0}; // Combine valid signals

    logic [NUM_LANES-1:0] fifo_push;                // Push signals for each FIFO
    logic [NUM_LANES-1:0] pre_fifo_pop;          // FIFO data valid signals
    logic [2*DICE_TID_WIDTH-1:0] fifo_push_data [NUM_LANES];      // Data to push into
    logic [NUM_LANES-1:0] fifo_pop_individual;      // Individual pop signals for each FIFO

    logic [NUM_LANES-1:0] full;

    logic [DICE_TID_WIDTH-1:0] fifo_full_tid [NUM_LANES];           // Data read from each FIFO
    ...
    generate
        for (i = 0; i < NUM_LANES; i++) begin : gen_fifos
            sync_fifo_read_unreg #(
                .DATA_WIDTH(2*DICE_TID_WIDTH),    // 2*DICE_TID_WIDTH bits: {compared_tid, real_tid} //compared tid for next stage
