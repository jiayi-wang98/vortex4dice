module simt_stack_controller #(
    ...
    output logic [NUM_STACK-1:0][dice_pkg::DICE_ADDR_WIDTH-1:0] stack_top_next_pc_o,
    output logic [NUM_STACK-1:0][dice_pkg::DICE_ADDR_WIDTH-1:0] stack_top_reconvergence_pc_o,
  ...
  logic [$clog2(NUM_STACK)-1:0] hw_cta_id_q;
  ...
  always_comb begin
    ...
    case (hw_cta_size_q)
      2'b00:
      effective_predicate = {
        {(NUM_STACK - 1) * THREAD_WIDTH{1'b0}}, predicate_regs_value_q[THREAD_WIDTH-1:0]
      };
      2'b01:
      effective_predicate = {
        {(NUM_STACK - 2) * THREAD_WIDTH{1'b0}}, predicate_regs_value_q[2*THREAD_WIDTH-1:0]
      };
      ...
      if ((current_state_q == StateReadTop) && (combined_stack_out_valid == 1'b1)) begin
        ...
        if (need_modify_top_next == 1'b1) begin
          $display(
              ...
              (update_with_divergence_q == 1'b0) ? update_next_pc_q :
              (all_taken == 1'b1) ? update_next_pc_q :
              (all_not_taken == 1'b1) ? branch_not_taken_pc_q : branch_reconvergence_pc_q);
        end
      end
    end
  end
